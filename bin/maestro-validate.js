...
function parseArgs(argv) {
  const args = { input: null, params: null, format: null, outputDir: null, failOnWarnings: false, saveZipFile: false };
...
(async () => {
  ...
  if (stat.isDirectory()) {
    ...
    try {
      zipBuffer = await directoryToZipBuffer(inputPath);
    } catch (e) {
  ...
  const results = await validateTestSuite(zipBuffer, zipFilename, buildParams);
